...
class GreedySolver:
    ...
    def heuristic(self, block):         # Distância de Manhattan
        ...
        return distance + orientation_penalty + danger_penalty  # + dist_gem
    ...
    def solve(self):
        ...
        # Priority queue for Greedy Search (using heap)
        # Each heap element is a tuple: (heuristic, counter, block, path)
        heap = []
        self.counter = 0
        initial_heuristic = self.heuristic(start_block)
        heapq.heappush(heap, (initial_heuristic, self.counter, start_block, []))
        self.counter += 1
        self.visited.add(self.get_state_key(start_block))
        
        while heap:
            _, _, current_block, path = heapq.heappop(heap)
            ...
            for move, x, y, state in self.get_possible_moves(current_block):
                ...
                if state_key not in self.visited:
                    ...
                    # Push to heap with heuristic as priority
                    new_heuristic = self.heuristic(new_block)
                    heapq.heappush(heap, (new_heuristic, self.counter, new_block, path + [move]))
                    self.counter += 1
...
if __name__ == "__main__":
    ...
    print("A executar Greedy Search...")
